    ...
    ui(new Ui::ShapeEditor)
{
    ...
    connect(this->ui->deleteShape, SIGNAL(clicked(bool)), this, SLOT(onDeleteClicked(bool)));
    connect(this->ui->shapeType, SIGNAL(currentIndexChanged(int)), this, SLOT(handleTextSelection(int)));
    if(shape != nullptr)
    {
        ...
        {
            ui->Square_id->setValue(shape->getId());
            ui->square_x1->setValue(shape->getPoint(0));
            ui->square_x1->setValue(shape->getPoint(1));
            ui->square_x1->setValue(shape->getPoint(2));
            ui->square_x1->setValue(shape->getPoint(3));
        ...
        {
            ui->circle_id->setValue(shape->getId());
            ui->circle_x1->setValue(shape->getPoint(0));
            ui->circle_x1->setValue(shape->getPoint(1));
            ui->circle_x1->setValue(shape->getPoint(2));
            ui->circle_x1->setValue(shape->getPoint(3));

            break;
        }
        case Shape<int>::ShapeType::text:
        {
            ui->text_id->setValue(shape->getId());
            break;
        }
        }
        // set pen/brush data
...
bool ShapeEditor::getDeleting() const
{
    return deleting;
}
...
void ShapeEditor::onDeleteClicked(bool)
{
    if (QMessageBox::question(this, "Warning!", "Are you sure you want to delete the current shape? (This cannot be undone!)") == QMessageBox::Yes)
    {
        deleting = true;
        reject();
    }
...
void ShapeEditor::handleTextSelection(int choice)
{
    if(choice == Shape<int>::ShapeType::text)
    {
        ui->penbrushdetails->setCurrentIndex(1);
    }
    else if (choice == Shape<int>::ShapeType::empty)
    {
        ui->penbrushdetails->setCurrentIndex(2);
    }
    else
    {
        ui->penbrushdetails->setCurrentIndex(0);
    }
}


Shape<int>* ShapeEditor::getShapeData() const
{
    ...
    case Shape<int>::ShapeType::square:
    {
        QPoint p1(ui->square_x1->value(), ui->square_y1->value());
        QPoint p2(ui->square_x2->value(), ui->square_y2->value());
        scrumptious::Vector<int> dimensions;
        dimensions.push_back(p1.x());
        dimensions.push_back(p1.y());
        dimensions.push_back(p2.x());
        ...
        Rectangle<int>* square = new Rectangle<int>(ui->Square_id->value(),
                                                    Shape<int>::ShapeType::square,
                                                    dimensions, pen);
        // missing square
        break;
    }
    ...
    {
        scrumptious::Vector<int> dimensions;
        dimensions.push_back(ui->circle_x1->value());
        dimensions.push_back(ui->circle_y1->value());
        dimensions.push_back(ui->circle_x2->value());
        dimensions.push_back(ui->circle_y2->value());

        Ellipse<int>* circle = new Ellipse<int>(ui->Ellipse_id->value(),
                                                Shape<int>::ShapeType::circle,
                                                dimensions, pen);
